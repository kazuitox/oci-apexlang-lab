...
ソース生成・検証、DBセットアップ、ADBへのアプリインポート、ブラウザでの動作確認は、それぞれ別工程として扱う。
...
- 既存テーブル・列・キーは提供DDLやメタデータで確認する。DBへ接続できなければ既存構造を推測しない。新規アプリ用のテーブル、ビュー、トリガー、パッケージ、権限、初期データは、それぞれDBセットアップSQLとして設計し、未適用であることを明示する。
- アプリ内のSQL（リージョン・プロセス・検証・LOV・共有コンポーネント等）を確認し、参照する表、ビュー、パッケージ、権限その他の依存物と、それらを作成・投入するセットアップSQLを特定する。アプリ定義だけから既存DB構造を推測しない。
...
ソース作成後の報告には、変更ファイル・実装機能・実際の検証結果・未適用のDBセットアップSQL・DBセットアップとアプリインポートの各実行状況を記載する。未実行の工程は、実際のファイルパス、SQLclで実行するかBashで実行するか、実行順序を明記して、作業者に実行を促す。

## アプリ作成後について

アプリが作成できたら、まずアプリ内のSQLからDB依存物とDBセットアップSQLを特定する。DBセットアップとアプリインポートは必ず別工程にし、Supporting Objectsの自動適用を前提にしない。インポートは手動で必要なので、Codexは対象アプリに合わせた手順を提示し、作業者に実行を促す。追加のインポート依頼を待たず、ソース作成・構文検証だけで作業完了としない。

## DBセットアップとアプリインポート（作業者が手動で実行）
...
認証が未準備ならDBセットアップとアプリインポートを未実行として報告する。ソース生成・編集・接続不要の検証を巻き戻したり、手動接続待ちにしたりしない。
...
既存ユーザーを削除・再作成したり、パスワードを勝手に変更したりしない。必要な初回設定はDBセットアップの範囲で扱う。
...
対象が解決できたら、作業者に次の順序で、実在するセットアップSQLとアプリディレクトリのパスを提示して実行を促す。存在しないファイルのパスや仮のSQLを実行指示にしない。
...
続いて、解析スキーマで接続したSQLcl内で、まず適用前の状態を確認する。対象名はアプリ内SQLと実在するセットアップSQLから特定した名前へ置換する：

```sql
select object_name, object_type, status
from user_objects
where object_name in ('<TABLE_OR_VIEW_OR_TRIGGER_OR_PACKAGE>')
...
次に、DBセットアップSQLを適用する。テーブルだけでなく、ビュー、トリガー、パッケージ、必要な権限、初期データをこの工程で明示的に適用する。Supporting Objectsの自動適用に依存しない：

```sql
@/home/opc/projects/apex-study/studyapp/db/setup.sql
```

`/home/opc/projects/apex-study/studyapp/db/setup.sql` は例であり、実際には確認済みのセットアップSQLの絶対パスへ置換する。適用後、同じ対象を再照合し、オブジェクトが期待する型・`VALID` 状態であること、初期データが期待件数または識別子で存在すること、必要な権限が満たされることを確認する。例：
...
select count(*) as initial_data_count
from <TABLE_NAME>;
```

DBセットアップの適用と照合が完了した後に限り、同じSQLcl接続内でアプリを検証・インポートする：
...
部分適用・再実行では、初回専用のユーザー作成、権限初期化、表作成、初期データ投入等のスクリプトを安易に再実行しない。実行済みの状態、スクリプトの冪等性、影響範囲を確認し、必要な差分SQLだけを対象として明示してから適用する。
DBセットアップ成功、アプリインポート成功、ブラウザ動作確認を区別し、未実行の工程を完了扱いにしない。
